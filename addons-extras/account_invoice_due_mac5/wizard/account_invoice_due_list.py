...
class InvoiceDueList(models.TransientModel):
    ...
    def list_due_invoices(self):
        ...
        if invoice_type in ('out_invoice', 'in_refund'):
            ...
            action['name'] = _('Customer invoices due on or before %s' % (self.date))
        else:
            action = self.env['ir.actions.act_window'].for_xml_id('account', 'action_invoice_tree2')
            action['name'] = _('Vendor invoices due on or before %s' % (self.date))
