...
class Configuration(models.Model):
    ...
    @api.model
    def scheduling(self, context=None):
        if not self:
    ...
    @api.model
    def schedule_s3_backup(self, context=None):
            ...
            if rec.name in db_list:
                ...
                try:
                    with open(file_path, 'wb') as fp:
                        backup = openerp.service.db.dump_db(rec.name, fp, rec.backup_type)
                        fp.close()
    ...
    def submit_values(self, file_path):
        ...
        filename = file_path
        des_filename = filename.splitlines()[0].split('/')[-1]
        sourcepath = filename
