...
class Parser(rml_parse):
    ...
    def __init__(self, cr, uid, name, context):
        ...
        self.company_currency_id = context.get('company_currency_id', False)
        self.currency_id = context.get('currency_id', False)
                    ...
                    if line.account_id.type == 'receivable':
                        if self . currency_id == line . currency_id . id:
                            if line . amount_currency > 0.0:
                                ...
                                credit += ( -1 * line . amount_currency )
                        else:
                            debit += currency_obj . compute ( self . cr, self . uid, self . company_currency_id, self . currency_id, line . debit, round = False, context = { 'date': line.date } )
                            credit += currency_obj . compute ( self . cr, self . uid, self . company_currency_id, self . currency_id, line . credit, round = False, context = { 'date': line.date } )
                elif self.result_selection == 'supplier':
                    if line.account_id.type == 'payable':
                        if self . currency_id == line . currency_id . id:
                            if line . amount_currency > 0.0:
    ...
    def get_initial_credit(self, partner, context=None):
        ...
        if self . currency_id == self . company_currency_id:
            ...
            # OJO !!! funciona solamente cuando todos los movimientos son en la moneda
            # seleccionada. En otro caso las cuentas no cerrarán.
            sql_stm = 'SELECT sum(l.amount_currency) ' \
                      'FROM account_move_line l, account_move m, account_account a '\
                      'WHERE l.move_id = m.id ' \
                'AND l.account_id = a.id ' \
                'AND l.partner_id = %s' \
                ...
                'AND l.amount_currency < 0.0 ' \
                      % ( partner.id, account_type, self.from_date, self . currency_id )
        ...
        if not res[0][0]:
            ...
            return res[0][0] < 0 and ( -1 * res[0][0] ) or res[0][0]
