...
class wizard_facturas_validadas_export(models.TransientModel):
    ...
    cliente_proveedor = fields.Selection([('out_invoice', 'Cliente'), ('in_invoice', 'Proveedor')], string="Tipo", default='cliente')
    ...
    def _onchange_nro_factura(self):
        if self.nro_factura:
            factura_rec=self.env['account.invoice'].search([('number','=',self.nro_factura.upper())])
            fact_list = []
            self.bandera = False
            if factura_rec:
    ...
    def action_export_validadas(self):
        ...
        #titulos
        ws.write(0,0,"Proveedor/Cliente",title_lime)
        ws.write(0,1,'RUT',title_lime)
        ws.write(0,2,'Nº de Factura',title_lime)
        ws.write(0,3,'Fecha de factura',title_lime)
        ws.write(0,4,'Fecha de Vencimiento',title_lime)
        ws.write(0,5,'Monto',title_lime)
        ws.write(0,6,'Celular',title_lime)
        ws.write(0,7,'Telefono',title_lime)
        ws.write(0,8,'Email',title_lime)
        ...
        if self.tipo_facturas == "pagas":
            sql='''   
                      SELECT rp.name, rp.vat, ai.number, ai.date_invoice, ai.date_due, ai.amount_total, rp.mobile,rp.phone, rp.email
                        ...
                        and  ai.date_invoice  >= DATE %(fecha_desde)s  and
                          ...
                          and ai.type = %(cliente_proveedor)s
        ...
        self.env.cr.execute(sql,
                            {'fecha_desde': self.fecha_inicial,
                             ...
                             'cliente_proveedor':self.cliente_proveedor
                             }
                            )
